...
import androidx.compose.ui.interop.InteropLayer
...
internal interface InteropContainer<T> {
    ...
    fun addInteropView(nativeView: T, layer: InteropLayer)
    fun removeInteropView(nativeView: T, layer: InteropLayer)
    fun getInteropView(layer: InteropLayer): Set<T>
}
 ...
 */
internal fun <T> InteropContainer<T>.countInteropComponentsBefore(nativeView: T, layer: InteropLayer): Int {
    ...
    rootModifier?.traverseDescendants {
        if (it.nativeView != nativeView) {
            ...
            if (it.nativeView in getInteropView(layer)) {
                componentsBefore++
            }
            ContinueTraversal
        } else {
