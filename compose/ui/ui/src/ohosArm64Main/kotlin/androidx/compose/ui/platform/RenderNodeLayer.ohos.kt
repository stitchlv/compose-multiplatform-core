...
import com.bytedance.kmp.harko.skia.Picture
import com.bytedance.kmp.harko.skia.PictureRecorder
import com.bytedance.kmp.harko.skia.Point3
import com.bytedance.kmp.harko.skia.Rect as NativeRect
import com.bytedance.kmp.harko.skia.RTreeFactory
import com.bytedance.kmp.harko.skia.ShadowUtils
...
internal class RenderNodeLayer(
    ...
    private val pictureRecorder = PictureRecorder().apply {
        if (disableRecycleNode) {
            disableUsingRecycleNode()
        }
    }
    ...
    override fun drawLayer(canvas: Canvas) {
        if (picture == null && ComposeController.isUsePictureRecorder) {
            ...
            if (requiresLayer) {
                ...
                canvas.saveLayer(
                    ...
                    Paint().apply {
                        ...
                        asFrameworkPaint().imageFilter = currentRenderEffect?.asHarkoImageFilter()
                    }
                )
            } else {
                canvas.save()
    ...
    private fun Canvas.clipRoundRect(rect: RoundRect, clipOp: ClipOp = ClipOp.Intersect) {
        val antiAlias = true
        nativeCanvas.clipRRect(rect.toHarko(), clipOp.toHarko(), antiAlias)
    }
    ...
    fun drawShadow(canvas: Canvas) = with(density) {
        ...
        ShadowUtils.drawShadow(
            canvas.nativeCanvas, path.toHarko(), zParams, lightPos,
            lightRad,
            ambientColor.toArgb(),
...
private val PICTURE_BOUNDS = NativeRect.makeLTRB(
    l = PICTURE_MIN_VALUE,
    t = PICTURE_MIN_VALUE,
    r = PICTURE_MAX_VALUE,
    b = PICTURE_MAX_VALUE
