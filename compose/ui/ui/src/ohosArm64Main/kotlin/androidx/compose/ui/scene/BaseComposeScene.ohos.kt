...
internal abstract class BaseComposeScene(
    ...
    protected fun invalidateIfNeeded() {
        ...
        if (isInitFinish && hasPendingDraws && !isInvalidationDisabled && !isClosed && composition != null) {
            invalidate()
        }
    }
    ...
    override fun sendPointerEvent(
        ...
        nativeEvent: Any?,
        button: PointerButton?,
    ) : ProcessResult = postponeInvalidation("BaseComposeScene:sendPointerEvent") {
        inputHandler.onPointerEvent(
            eventType = eventType,
            pointers = pointers,
            buttons = buttons,
    ...
    protected abstract fun processPointerInputEvent(event: PointerInputEvent): ProcessResult
