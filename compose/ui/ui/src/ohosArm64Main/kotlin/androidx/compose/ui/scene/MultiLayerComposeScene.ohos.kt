...
internal fun MultiLayerComposeScene(
    ...
    invalidate: () -> Unit = {},
    disableRecycleNode: Boolean = false,
): MultiLayerComposeSceneImpl = MultiLayerComposeSceneImpl(
    density = density,
    ...
    disableRecycleNode = disableRecycleNode,
)

internal class MultiLayerComposeSceneImpl(
    density: Density,
    layoutDirection: LayoutDirection,
    size: IntSize?,
    coroutineContext: CoroutineContext,
    composeSceneContext: ComposeSceneContext,
    invalidate: () -> Unit = {},
    private val disableRecycleNode: Boolean = false,
) : BaseComposeScene(
    coroutineContext = coroutineContext,
    composeSceneContext = composeSceneContext,
    invalidate = invalidate
) {
    val mainOwner = RootNodeOwner(
        density = density,
        layoutDirection = layoutDirection,
        size = size,
    ...
    fun getCurrentCompositionData(): MutableSet<CompositionData> {
        return mainOwner.compositionDataSet
    }

    fun getRootForTest(): RootForTest {
        return mainOwner.getRootForTest()
    }
    ...
    override fun processPointerInputEvent(event: PointerInputEvent): ProcessResult {
    ...
    private fun processPress(event: PointerInputEvent): ProcessResult {
        ...
        forEachLayerReversed { layer ->
            ...
            val isHandled = layer.onOutsidePointerEvent(event)

            // if the owner is in focus, do not pass the event to underlying owners
            if (layer == focusedLayer) {
                return ProcessResult(isHandled, anyMovementConsumed = false)
            }
        }
    ...
    private fun processRelease(event: PointerInputEvent): ProcessResult {
        ...
        if (!event.isGestureInProgress) {
            ...
            if (isInteractive(owner)) {
                isHandled = processHover(event, owner)
                ...
                focusedLayer?.let {
                    isHandled = it.onOutsidePointerEvent(event)
                }
            }
        }
        return ProcessResult(
            result?.dispatchedToAPointerInputModifier ?: isHandled,
            result?.anyMovementConsumed ?: false
        )
    }

    private fun processMove(event: PointerInputEvent): ProcessResult {
        var owner = when {
        ...
        if (processHover(event, owner)) {
            return ProcessResult(true, anyMovementConsumed = false)
        }
        return owner?.onPointerInput(event.copy(eventType = PointerEventType.Move)) ?: ProcessResult(0)
    }
    ...
    private fun processScroll(event: PointerInputEvent): ProcessResult {
        val owner = hoveredOwner(event)
        if (isInteractive(owner)) {
            return owner.onPointerInput(event)
        }
    ...
    private inner class AttachedComposeSceneLayer(
        ...
        fun onOutsidePointerEvent(event: PointerInputEvent): Boolean {
            if (!event.isMouseOrSingleTouch()) {
                return false
            }
            return outsidePointerCallback?.run {
                invoke(event.eventType, event.button)
                true
            } ?: false
        }
    }
