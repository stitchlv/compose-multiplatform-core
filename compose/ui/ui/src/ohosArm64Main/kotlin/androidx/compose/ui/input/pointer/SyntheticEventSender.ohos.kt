...
internal class SyntheticEventSender(
    send: (PointerInputEvent) -> ProcessResult
) {
    private val _send: (PointerInputEvent) -> ProcessResult = send
     ...
     */
    fun send(event: PointerInputEvent): ProcessResult {
        val hoverResult = sendMissingMoveForHover(event)
        val releaseResult = sendMissingReleases(event)
        val pressResult = sendMissingPresses(event)
        val result = sendInternal(event)
        return ProcessResult(
            hoverResult.dispatchedToAPointerInputModifier || releaseResult.dispatchedToAPointerInputModifier || pressResult.dispatchedToAPointerInputModifier || result.dispatchedToAPointerInputModifier,
            hoverResult.anyMovementConsumed || releaseResult.anyMovementConsumed || pressResult.anyMovementConsumed || result.anyMovementConsumed
        )
    }
     ...
     */
    private fun sendSyntheticMove(pointersSourceEvent: PointerInputEvent): ProcessResult {
        val previousEvent = previousEvent ?: return ProcessResult(0)
    ...
    private fun sendMissingMoveForHover(currentEvent: PointerInputEvent): ProcessResult {
        ...
        if (currentEvent.pointers.any { it.issuesEnterExit } &&
            ...
            return sendSyntheticMove(currentEvent)
        }
        return ProcessResult(0)
    }

    private fun sendMissingReleases(currentEvent: PointerInputEvent): ProcessResult {
        val previousEvent = previousEvent ?: return ProcessResult(0)
        ...
        // before a real one.
        var dispatchedToAPointerInputModifier = false
        ...
        for (i in newReleased.size - 2 downTo 0) {
            ...
            if (!dispatchedToAPointerInputModifier) dispatchedToAPointerInputModifier = result.dispatchedToAPointerInputModifier
            if (!anyMovementConsumed) anyMovementConsumed = result.anyMovementConsumed
        }
        return ProcessResult(dispatchedToAPointerInputModifier, anyMovementConsumed)
    }

    private fun sendMissingPresses(currentEvent: PointerInputEvent): ProcessResult {
    ...
    private fun sendInternal(event: PointerInputEvent): ProcessResult {
