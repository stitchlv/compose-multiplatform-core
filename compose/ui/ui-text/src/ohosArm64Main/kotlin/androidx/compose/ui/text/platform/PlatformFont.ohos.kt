...
import com.bytedance.kmp.harko.skia.Data
import com.bytedance.kmp.harko.skia.FontMgr
import com.bytedance.kmp.harko.skia.FontSlant
import com.bytedance.kmp.harko.skia.FontStyle as SkFontStyle
import com.bytedance.kmp.harko.skia.FontWidth
import com.bytedance.kmp.harko.skia.Typeface as SkTypeface
import com.bytedance.kmp.harko.skia.paragraph.FontCollection
import com.bytedance.kmp.harko.skia.paragraph.TypefaceFontProvider
...
internal class FontCache {
    ...
    internal fun loadPlatformTypes(
        ...
        val typeface = fonts.findTypefaces(aliases?.toTypedArray(), style).first()
        return FontLoadResult(typeface, aliases ?: listOf(typeface!!.familyName))
    }
...
internal fun currentPlatform(): Platform = Platform.OHOS
internal fun loadTypeface(font: Font): SkTypeface {
    if (font !is PlatformFont) {
        throw IllegalArgumentException("Unsupported font type: $font")
    }
    return when (font) {
...
private val GenericFontFamiliesMapping: Map<String, List<String>> by lazy {
    when (currentPlatform()) {
        ...
        Platform.OHOS-> mapOf(
            FontFamily.SansSerif.name to listOf("HarmonyOS-Sans"),
            FontFamily.Serif.name to listOf("serif"),
            FontFamily.Monospace.name to listOf("monospace"),
            FontFamily.Cursive.name to listOf("HarmonyOS-Sans") // not support,use default
