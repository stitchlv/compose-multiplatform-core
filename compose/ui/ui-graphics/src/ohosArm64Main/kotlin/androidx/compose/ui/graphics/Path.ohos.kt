...
import com.bytedance.kmp.harko.skia.Matrix33
import com.bytedance.kmp.harko.skia.Path as NativePath
import com.bytedance.kmp.harko.skia.PathDirection
import com.bytedance.kmp.harko.skia.PathFillMode
import com.bytedance.kmp.harko.skia.PathOp

actual fun Path(): Path = HarkoPath()

fun Path.toHarko(): NativePath = (this as HarkoPath).native
fun NativePath.asComposePath(): Path = HarkoPath(this)

internal class HarkoPath(var native: NativePath = NativePath()) : Path {
    override var fillType: PathFillType
        get() {
            return if (native.fillMode == PathFillMode.EVEN_ODD) {
        ...
        set(value) {
            native.fillMode = if (value == PathFillType.EvenOdd) {
    ...
    override fun relativeQuadraticBezierTo(dx1: Float, dy1: Float, dx2: Float, dy2: Float) {
        native.rQuadTo(dx1, dy1, dx2, dy2)
    }
    ...
    override fun relativeCubicTo(
        ...
        native.rCubicTo(dx1, dy1, dx2, dy2, dx3, dy3)
    }

    override fun arcTo(
        rect: Rect, startAngleDegrees: Float, sweepAngleDegrees: Float, forceMoveTo: Boolean
    ) {
        native.arcTo(rect.toHarko(), startAngleDegrees, sweepAngleDegrees, forceMoveTo)
    }

    override fun addRect(rect: Rect) {
        native.addRect(rect.toHarko(), PathDirection.COUNTER_CLOCKWISE)
    }

    override fun addOval(oval: Rect) {
        native.addOval(oval.toHarko(), PathDirection.COUNTER_CLOCKWISE)
    }
    ...
    override fun addArc(oval: Rect, startAngleDegrees: Float, sweepAngleDegrees: Float) {
        native.addArc(oval.toHarko(), startAngleDegrees, sweepAngleDegrees)
    }

    override fun addRoundRect(roundRect: RoundRect) {
        native.addRRect(roundRect.toHarko(), PathDirection.COUNTER_CLOCKWISE)
    }

    override fun addPath(path: Path, offset: Offset) {
        native.addPath(path.toHarko(), offset.x, offset.y)
    }
    ...
    override fun translate(offset: Offset) {
        native.transform(Matrix33.makeTranslate(offset.x, offset.y))
    }

    override fun transform(matrix: Matrix) {
        native.transform(Matrix33.makeTranslate(0f, 0f).apply { setFrom(matrix) })
    }

    override fun getBounds(): Rect {
        return native.bounds.toCompose()
    }

    override fun op(path1: Path, path2: Path, operation: PathOperation): Boolean {
        val path = NativePath.makeCombining(
            path1.toHarko(), path2.toHarko(), operation.toHarko()
        )
    ...
    override val isConvex: Boolean get() = native.isConvex

    override val isEmpty: Boolean get() = native.isEmpty
...
private fun PathOperation.toHarko() = when (this) {
